(function (root, factory) {
    ...
    if (root) root.OptionMapQriOptionsLastValidRestore = api;
    ...
    function parseQriOptionsLastValidCache(serialized) {
    ...
    async function restoreQriOptionsLastValidCache(input) {
        let candidate; let inputType; let parsed = false;
        if (typeof input === "string" || input === null || input === undefined) {
            const result = parseQriOptionsLastValidCache(input);
        ...
        if (!cacheApi?.validateQriOptionsLastValidCache) {
            return failure("validator_unavailable", inputType, { parsed });
        }
        let valid = false;
        try {
            valid = await cacheApi.validateQriOptionsLastValidCache(isolated);
        } catch (_error) {
    ...
    async function restoreQriOptionsLastValidWithFreshness(input, context = {}) {
        const restored = await restoreQriOptionsLastValidCache(input);
        if (!restored.success) {
    ...
    return Object.freeze({ RESTORE_VERSION, parseQriOptionsLastValidCache,
        restoreQriOptionsLastValidCache, restoreQriOptionsLastValidWithFreshness,
        deepFreeze });
});
